//#region src/locales/zh-TW/translations.ts
var translations_default = {
	...
	auth: {
		...
		sign_up: {
			...
			errors: { password: { strength: "請設定強密碼以繼續" } }
		},
		sign_in: { header: {
			label: "登入開始與團隊一起管理工作。",
			step: {
		...
		forgot_password: {
			...
			errors: { smtp_not_enabled: "我們發現您的管理員尚未啟用SMTP，我們將無法發送密碼重設連結" },
			toast: {
				success: {
					title: "郵件已發送",
	...
	home: {
		...
		new_at_plane: { title: "Plane 新功能" },
		quick_tutorial: { title: "快速教學" },
		widget: {
	...
	issue: {
		...
		remove: {
			...
			parent: { label: "移除父工作事項" }
		},
		new: "新工作事項",
		adding: "新增工作事項中",
		create: { success: "工作事項建立成功" },
		priority: {
			urgent: "緊急",
			high: "高",
			medium: "中",
		...
		empty_state: { issue_detail: {
			...
			primary_button: { text: "檢視其他工作事項" }
		} },
		sibling: { label: "同層級工作事項" },
		archive: {
	...
	workspace_projects: {
		...
		common: { months_count: "{months, plural, one{# 個月} other{# 個月}}" },
		empty_state: {
			general: {
				title: "沒有使用中的專案",
			...
			search: { description: "找不到符合篩選條件的專案。\n改為建立新專案" }
		}
	},
	workspace_views: {
		add_view: "新增檢視",
		...
		settings: {
			...
			webhooks: {
				...
				toasts: {
					...
					secret_key_copied: { message: "金鑰已複製到剪貼簿。" },
					secret_key_not_copied: { message: "複製金鑰時發生錯誤。" }
				}
			},
			api_tokens: {
				title: "API 權杖",
	...
	project_cycles: {
		...
		upcoming_cycle: { label: "即將到來的週期" },
		completed_cycle: { label: "已完成的週期" },
		status: {
			days_left: "剩餘天數",
			completed: "已完成",
			yet_to_start: "尚未開始",
		...
		action: {
			...
			update: {
				...
				error: { already_exists: "給定日期範圍內已有週期，如果您要建立草稿週期，可以移除兩個日期來執行此操作。" }
			}
		},
		empty_state: {
			general: {
			...
			no_issues: {
				...
				primary_button: { text: "建立新工作事項" },
				secondary_button: { text: "新增現有工作事項" }
			},
			completed_no_issues: {
				title: "週期中沒有工作事項",
	...
	project_issues: { empty_state: {
		...
		no_archived_issues: {
			...
			primary_button: { text: "設定自動化" }
		},
		issues_empty_filter: {
			title: "找不到符合套用篩選器的工作事項",
			secondary_button: { text: "清除所有篩選器" }
		}
	} },
	project_module: {
		add_module: "新增模組",
	...
	command_k: { empty_state: { search: { title: "找不到結果" } } },
	issue_relation: { empty_state: {
		search: { title: "找不到符合的工作事項" },
		no_issues: { title: "找不到工作事項" }
	} },
	...
	active_cycle: { empty_state: {
		progress: { title: "新增工作事項到週期以檢視其進度" },
		chart: { title: "新增工作事項到週期以檢視燃盡圖。" },
		priority_issue: { title: "快速檢視週期中處理的高優先順序工作事項。" },
		assignee: { title: "新增指派對象到工作事項以檢視依指派對象分類的工作分析。" },
		label: { title: "新增標籤到工作事項以檢視依標籤分類的工作分析。" }
	} },
	disabled_project: { empty_state: {
		inbox: {
			title: "進件功能未啟用於此專案。",
			description: "進件可協助您管理專案的傳入請求，並將它們新增為工作流程中的工作事項。從專案設定啟用進件以管理請求。",
			primary_button: { text: "管理功能" }
		},
		cycle: {
			title: "週期功能未啟用於此專案。",
	...
	themes: { theme_options: {
		system_preference: { label: "系統偏好設定" },
		light: { label: "淺色" },
		dark: { label: "深色" },
		light_contrast: { label: "高對比淺色" },
		dark_contrast: { label: "高對比深色" },
		custom: { label: "自訂主題" }
	} },
	project_modules: {
		status: {
...
//# sourceMappingURL=translations-CcNuvddF.js.map