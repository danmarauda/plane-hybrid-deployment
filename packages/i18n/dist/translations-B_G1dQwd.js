//#region src/locales/zh-CN/translations.ts
var translations_default = {
	...
	auth: {
		...
		sign_up: {
			...
			errors: { password: { strength: "请设置一个强密码以继续" } }
		},
		sign_in: { header: {
		...
		forgot_password: {
			...
			errors: { smtp_not_enabled: "我们发现您的管理员未启用 SMTP，我们将无法发送密码重置链接" },
			toast: {
				success: {
					title: "邮件已发送",
	...
	home: {
		...
		new_at_plane: { title: "Plane新功能" },
		quick_tutorial: { title: "快速教程" },
		widget: {
	...
	workspace_projects: {
		...
		common: { months_count: "{months, plural, one{# 个月} other{# 个月}}" },
		empty_state: {
			general: {
				title: "没有活动项目",
			...
			search: { description: "未检测到符合匹配条件的项目。\n创建一个新项目" }
		}
	},
	workspace_views: {
		add_view: "添加视图",
		...
		settings: {
			...
			webhooks: {
				...
				toasts: {
					...
					secret_key_copied: { message: "密钥已复制到剪贴板。" },
					secret_key_not_copied: { message: "复制密钥时出错。" }
				}
			},
			api_tokens: {
				title: "API 令牌",
	...
	project_cycles: {
		...
		upcoming_cycle: { label: "即将到来的周期" },
		completed_cycle: { label: "已完成的周期" },
		status: {
			days_left: "剩余天数",
			completed: "已完成",
			yet_to_start: "尚未开始",
		...
		action: {
			...
			update: {
				...
				error: { already_exists: "在给定日期范围内已存在周期，如果您想创建草稿周期，可以通过移除两个日期来实现。" }
			}
		},
		empty_state: {
			general: {
	...
	project_issues: { empty_state: {
		...
		no_archived_issues: {
			...
			primary_button: { text: "设置自动化" }
		},
		issues_empty_filter: {
			title: "未找到符合筛选条件的工作项",
			secondary_button: { text: "清除所有筛选条件" }
		}
	} },
	project_module: {
		add_module: "添加模块",
	...
	command_k: { empty_state: { search: { title: "未找到结果" } } },
	issue_relation: { empty_state: {
		search: { title: "未找到匹配的工作项" },
		no_issues: { title: "未找到工作项" }
	} },
	...
	active_cycle: { empty_state: {
		...
		chart: { title: "向周期添加工作项以查看燃尽图。" },
		priority_issue: { title: "一目了然地观察周期中处理的高优先级工作项。" },
		assignee: { title: "为工作项添加负责人以查看按负责人划分的工作明细。" },
		label: { title: "为工作项添加标签以查看按标签划分的工作明细。" }
	} },
	...
	themes: { theme_options: {
		system_preference: { label: "系统偏好" },
		light: { label: "浅色" },
		dark: { label: "深色" },
		light_contrast: { label: "浅色高对比度" },
		dark_contrast: { label: "深色高对比度" },
		custom: { label: "自定义主题" }
	} },
	project_modules: {
		status: {
...
//# sourceMappingURL=translations-B_G1dQwd.js.map