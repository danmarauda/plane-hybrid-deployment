//#region src/locales/vi-VN/translations.ts
var translations_default = {
	...
	auth: {
		common: {
			...
			password: {
				...
				current_password: { label: "Mật khẩu hiện tại" },
				new_password: {
					label: "Mật khẩu mới",
		...
		sign_up: {
			...
			errors: { password: { strength: "Vui lòng đặt mật khẩu mạnh để tiếp tục" } }
		},
		sign_in: { header: {
		...
		forgot_password: {
			...
			errors: { smtp_not_enabled: "Chúng tôi nhận thấy quản trị viên của bạn chưa bật SMTP, chúng tôi sẽ không thể gửi liên kết đặt lại mật khẩu" },
			toast: {
				success: {
	...
	home: {
		empty: {
			...
			widgets: {
				...
				primary_button: { text: "Quản lý tiện ích" }
			}
		},
		quick_links: {
		...
		new_at_plane: { title: "Tính năng mới của Plane" },
		quick_tutorial: { title: "Hướng dẫn nhanh" },
		widget: {
	...
	entity: {
		...
		fetch: { failed: "Đã xảy ra lỗi khi tải {entity}" },
		add: {
	...
	epic: {
		...
		create: { success: "Đã tạo sử thi thành công" },
		add: {
	...
	issue: {
		...
		remove: {
			...
			parent: { label: "Xóa mục công việc cha" }
		},
		new: "Mục công việc mới",
		adding: "Đang thêm mục công việc",
		create: { success: "Đã tạo mục công việc thành công" },
		priority: {
			urgent: "Khẩn cấp",
			high: "Cao",
			medium: "Trung bình",
		...
		comments: {
			...
			upload: { error: "Không thể tải lên tài nguyên. Vui lòng thử lại sau." },
			copy_link: {
		...
		empty_state: { issue_detail: {
			...
			primary_button: { text: "Xem các mục công việc khác" }
		} },
		sibling: { label: "Mục công việc cùng cấp" },
		archive: {
			...
			failed: { message: "Không thể lưu trữ mục công việc. Vui lòng thử lại." }
		},
		restore: {
			success: {
			...
			failed: { message: "Không thể khôi phục mục công việc. Vui lòng thử lại." }
		},
		relation: {
	...
	view: {
		...
		create: { label: "Tạo chế độ xem" },
		update: { label: "Cập nhật chế độ xem" }
	},
	inbox_issue: {
		status: {
			pending: {
		...
		modal: { title: "Tạo mục công việc thu thập" },
		tabs: {
			open: "Chưa xử lý",
			closed: "Đã xử lý"
		},
		empty_state: {
			...
			detail: { title: "Chọn một mục công việc để xem chi tiết." }
		}
	},
	workspace_creation: {
	...
	workspace_projects: {
		...
		common: { months_count: "{months, plural, one{# tháng} other{# tháng}}" },
		empty_state: {
			general: {
			...
			search: { description: "Không phát hiện dự án nào phù hợp với điều kiện tìm kiếm.\nTạo dự án mới" }
		}
	},
	workspace_views: {
		...
		empty_state: {
			"all-issues": {
				...
				primary_button: { text: "Tạo mục công việc mới" }
			},
			assigned: {
		...
		delete_view: {
			...
			content: "Nếu bạn xác nhận, tất cả các tùy chọn sắp xếp, lọc và hiển thị + bố cục mà bạn đã chọn cho chế độ xem này sẽ bị xóa vĩnh viễn mà không có cách nào khôi phục."
		}
	},
		...
		settings: {
			...
			webhooks: {
				...
				toasts: {
					...
					secret_key_copied: { message: "Đã sao chép khóa bí mật vào bảng tạm." },
					secret_key_not_copied: { message: "Đã xảy ra lỗi khi sao chép khóa bí mật." }
				}
			},
			api_tokens: {
				title: "Token API",
	...
	project_settings: {
		...
		labels: {
			...
			toast: { error: "Đã xảy ra lỗi khi cập nhật nhãn" }
		},
		estimates: {
			label: "Ước tính",
	...
	project_cycles: {
		...
		upcoming_cycle: { label: "Chu kỳ sắp tới" },
		completed_cycle: { label: "Chu kỳ đã hoàn thành" },
		status: {
		...
		action: {
			...
			update: {
				...
				error: { already_exists: "Đã tồn tại chu kỳ trong khoảng thời gian đã cho, nếu bạn muốn tạo chu kỳ nháp, bạn có thể làm vậy bằng cách xóa cả hai ngày." }
			}
		},
		empty_state: {
			general: {
			...
			no_issues: {
				...
				primary_button: { text: "Tạo mục công việc mới" },
				secondary_button: { text: "Thêm mục công việc hiện có" }
			},
			completed_no_issues: {
	...
	project_issues: { empty_state: {
		...
		no_archived_issues: {
			...
			primary_button: { text: "Thiết lập tự động hóa" }
		},
		issues_empty_filter: {
			title: "Không tìm thấy mục công việc phù hợp với bộ lọc",
			secondary_button: { text: "Xóa tất cả bộ lọc" }
		}
	} },
	project_module: {
		...
		toast: {
			copy: { success: "Đã sao chép liên kết mô-đun vào bảng tạm" },
			delete: {
	...
	project_page: { empty_state: {
		general: {
			...
			primary_button: { text: "Tạo trang đầu tiên của bạn" }
		},
		private: {
	...
	command_k: { empty_state: { search: { title: "Không tìm thấy kết quả" } } },
	issue_relation: { empty_state: {
		search: { title: "Không tìm thấy mục công việc phù hợp" },
		no_issues: { title: "Không tìm thấy mục công việc" }
	} },
	...
	notification: {
		...
		empty_state: {
			detail: { title: "Chọn để xem chi tiết." },
			all: {
	...
	active_cycle: { empty_state: {
		progress: { title: "Thêm mục công việc vào chu kỳ để xem tiến độ của nó" },
		chart: { title: "Thêm mục công việc vào chu kỳ để xem biểu đồ burndown." },
		priority_issue: { title: "Xem nhanh các mục công việc ưu tiên cao đang được xử lý trong chu kỳ." },
		assignee: { title: "Thêm người phụ trách cho mục công việc để xem phân tích công việc theo người phụ trách." },
		label: { title: "Thêm nhãn cho mục công việc để xem phân tích công việc theo nhãn." }
	} },
	disabled_project: { empty_state: {
		inbox: {
			...
			primary_button: { text: "Quản lý tính năng" }
		},
		cycle: {
	...
	workspace_draft_issues: {
		...
		empty_state: {
			...
			primary_button: { text: "Tạo bản nháp đầu tiên của bạn" }
		},
		delete_modal: {
			title: "Xóa bản nháp",
		...
		toasts: {
			...
			deleted: { success: "Đã xóa bản nháp" }
		}
	},
	stickies: {
		...
		empty_state: {
			...
			general: {
				...
				primary_button: { text: "Thêm ghi chú" }
			},
			search: {
	...
	themes: { theme_options: {
		system_preference: { label: "Tùy chọn hệ thống" },
		light: { label: "Sáng" },
		dark: { label: "Tối" },
		light_contrast: { label: "Sáng tương phản cao" },
		dark_contrast: { label: "Tối tương phản cao" },
		custom: { label: "Chủ đề tùy chỉnh" }
	} },
	project_modules: {
		status: {
			backlog: "Tồn đọng",
...
//# sourceMappingURL=translations-B62tteMh.js.map